...
public class BinarySearchTree<T extends Comparable<T>> {
	...
	private void preOrderRecurse(BSTNode<T> node) {
		...
		System.out.print(node.data + " " ); //first print data of node
		
		preOrderRecurse(node.leftChild); //then recur left children subtree
		
		preOrderRecurse(node.rightChild); //then recur right children subtree
	...
	public void preOrderStack() {
		...
		//run until reach leaf node or stack is empty
		while(current != null || !pre.isEmpty()) {
			//until reach a leaf node, print (root &) left children & push right children if they exist
			while (current != null) {
			...
			//once reach leaf node, pop last right child in stack & let it be the current node
			if (!pre.isEmpty()) {
				current = pre.pop();
			}
	...
	public void postOrderRecurse(BSTNode<T> node) {
		...
		postOrderRecurse(node.leftChild); //first recur left children subtree

		postOrderRecurse(node.rightChild); //then recur right children subtree

		System.out.print(node.data + " " ); //then print data of node
	...
	public void postOrderStack() {
		...
		if(root!=null) {
			...
			while(!postHelper.isEmpty()) {
				...
				//pop top node in postHelper & push it into post
				BSTNode<T> node = postHelper.pop();
				post.push(node);
				
				//push left then right children of node popped if not null
